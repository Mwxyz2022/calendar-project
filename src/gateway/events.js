...
export const fetchEvent = () =>
  fetch(EVENTS_API_URL).then(response => {
    if (!response.ok) {
...
export const postEvent = eventData =>
  fetch(EVENTS_API_URL, {
    ...
    if (!response.ok) {
      throw new Error(`Failed to create event. Status: ${response.status} ${response.statusText}`);
    }
...
export const deleteEvent = eventId =>
  fetch(`${EVENTS_API_URL}/${eventId}`, {
    ...
    if (!response.ok) {
      throw new Error(`Failed to delete event. Status: ${response.status} ${response.statusText}`);
    }
